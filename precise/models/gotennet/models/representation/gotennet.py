...
import precise.models.gotennet.utils as utils
from precise.models.gotennet.models.components.layers import (
    MLP,
    CosineCutoff,
    Dense,
    Distance,
...
class GATA(MessagePassing):
    ...
    def forward(
        ...
        if torch.isnan(h).any():
            print(f"[GATA] h NaN locations: {torch.isnan(h).sum()}")
        if torch.isnan(X).any():
            print(f"[GATA] X NaN locations: {torch.isnan(X).sum()}")

        h = self.layernorm(h)
        X = self.tensor_layernorm(X)

        if torch.isnan(h).any():
            print(f"[GATA] h NaN locations after norm: {torch.isnan(h).sum()}")
        if torch.isnan(X).any():
            print(f"[GATA] X NaN locations after norm: {torch.isnan(X).sum()}")
        ...
        if torch.isnan(d_h).any():
            print(f"[GATA] d_h NaN locations: {torch.isnan(d_h).sum()}")
        if torch.isnan(d_X).any():
            print(f"[GATA] d_X NaN locations: {torch.isnan(d_X).sum()}")

        h = h + d_h
        X = X + d_X

        if torch.isnan(h).any():
            print(f"[GATA] h final NaN locations: {torch.isnan(h).sum()}")
        if torch.isnan(X).any():
            print(f"[GATA] X final NaN locations: {torch.isnan(X).sum()}")
    ...
    def message(
        ...
        if torch.isnan(n_edges).any():
            print(f"[GATA] n_edges NaN locations: {torch.isnan(n_edges).sum()}")
        ...
        cutoff_vals = self.cutoff(r_ij.unsqueeze(-1).unsqueeze(-1))

        # Apply spatial filter
        spatial_attn = t_ij_filter.unsqueeze(1) * x_j * cutoff_vals
        ...
        if torch.isnan(dX).any():
            print(f"[GATA Message] dX NaN locations: {torch.isnan(dX).sum()}")
        return o_s_ij, dX
...
class EQFF(nn.Module):
    ...
    def forward(self, h: Tensor, X: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        if torch.isnan(h).any():
            print(f"[EQFF] h NaN locations: {torch.isnan(h).sum()}")
        if torch.isnan(X).any():
            print(f"[EQFF] X NaN locations: {torch.isnan(X).sum()}")
        ...
        if torch.isnan(h).any():
            print(f"[EQFF] h final NaN locations: {torch.isnan(h).sum()}")
        if torch.isnan(X).any():
            print(f"[EQFF] X final NaN locations: {torch.isnan(X).sum()}")
...
class GotenNet(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            edge_index: Tensor describing graph connectivity [2, num_edges]
            edge_diff: Tensor of edge distances [num_edges] ## Mert: Changed from [num_edges, 1] to [num_edges]
...
class GotenNetV2(GotenNet):
    """
    Variant of GotenNet that accepts per-node input features [num_nodes, n_feats]
    instead of atom indices, and projects them to `n_atom_basis` via an MLP.

    It also uses `NodeInitFeat` to perform environment-aware initialization using
    these node features as the source signals in message passing, replacing the
    embedding-based `NodeInit` used in `GotenNet`.
    """
    ...
    def __init__(
        ...
        # Construct base without NodeInit/embeddings, then override pieces
        super(GotenNet, self).__init__()
        ...
        # Project input node features to hidden dim
        self.node_projector = MLP(
            [n_input_node_feats, n_atom_basis],
            activation=None,
            norm="",
            weight_init=weight_init,
            bias_init=bias_init,
        ...
        # Node/Edge init using features instead of embeddings
        self.node_init = NodeInitFeat(
            [self.hidden_dim, self.hidden_dim],
            n_rbf,
            self.cutoff,
            weight_init=weight_init,
    ...
    def forward(
        ...
        """
        Args:
            node_features: [num_nodes, n_input_node_feats]
            edge_index: [2, num_edges]
            edge_diff: [num_edges]
            edge_vec: [num_edges, 3]
        Returns:
            h: [num_nodes, hidden_dims]
            X: [num_nodes, ((lmax+1)^2 - 1), hidden_dims]
        """

        h = self.node_projector(node_features)

        phi_r0_ij = self.radial_basis(edge_diff)

        h = self.node_init(h, edge_index, edge_diff, phi_r0_ij)

        t_ij_init = self.edge_init(edge_index, phi_r0_ij, h)
        mask = edge_index[0] != edge_index[1]
        r0_ij = torch.norm(edge_vec[mask], dim=1).unsqueeze(1)

        if torch.isnan(r0_ij).any():
            print(f"[GotenNetV2] r0_ij NaN locations: {torch.isnan(r0_ij).sum()}")

        # Fix: Handle zero or near-zero edge norms to prevent division by zero
        r0_ij = torch.clamp(r0_ij, min=1e-8)  # Prevent division by zero
        edge_vec[mask] = edge_vec[mask] / r0_ij

        # Fix: Handle invalid edge vectors that could cause NaN in spherical harmonics
        edge_norms = torch.norm(edge_vec, dim=1)
        invalid_edges = (edge_norms < 1e-8) | torch.isnan(edge_norms)
        if invalid_edges.any():
            print(f"[GotenNetV2] Found {invalid_edges.sum()} invalid edge vectors")
            # Replace invalid vectors with unit vectors
            edge_vec[invalid_edges] = torch.tensor(
                [1.0, 0.0, 0.0], device=edge_vec.device
            )
            print(f"[GotenNetV2] Replaced invalid edge vectors with [1,0,0]")

        rl_ij = self.sphere(edge_vec)[:, 1:]
        if torch.isnan(rl_ij).any():
            print(f"[GotenNetV2] rl_ij NaN locations: {torch.isnan(rl_ij).sum()}")
            # Additional fix: Replace NaN values in rl_ij with zeros
            rl_ij = torch.nan_to_num(rl_ij, nan=0.0)
...
class GotenNetV3(GotenNet):
    ...
    def __init__(
        ...
        if use_concise:
            self.drug_to_gate = nn.Sequential(
                nn.Embedding(32**3, self.n_atom_basis * self.n_interactions * 2),
                nn.SiLU(),
                nn.Linear(
                    self.n_atom_basis * self.n_interactions * 2,
                    self.n_atom_basis * self.n_interactions * 2,
                ),
            )
        else:
            self.drug_to_gate = nn.Sequential(
                nn.Linear(n_drug_feats, self.n_atom_basis),
                nn.SiLU(),
                nn.Linear(
                    self.n_atom_basis, self.n_atom_basis * self.n_interactions * 2
                ),
            )
    ...
    def forward(
        ...
        drug_dim = drug.size(0) // batch_size
        drug_features_batched = drug.view(batch_size, drug_dim)

        gate_params = self.drug_to_gate(
            drug_features_batched
        )  # [B, n_atom_basis * n_interactions * 2]
        gate_params = gate_params.view(
            batch_size, self.n_interactions, self.n_atom_basis, 2
        )
        gates, biases = (
            gate_params[..., 0],
            gate_params[..., 1],
        )  # [B, n_interactions, n_atom_basis]
        ...
        for _i, (gata, eqff) in enumerate(
            ...
            node_gates = gates[batch, _i]  # [num_nodes, n_atom_basis]
            node_biases = biases[batch, _i]  # [num_nodes, n_atom_basis]
            h = h * node_gates.unsqueeze(1) + node_biases.unsqueeze(1)
...
def find_degenerate_faces(data: Data, tol=1e-9) -> torch.Tensor:
    """
    Finds degenerate faces in a PyG Data object.
    A face is degenerate if the cross-product of its edge vectors is zero.

    Args:
        data (Data): The PyG data object with `pos` and `face` attributes.
        tol (float): A tolerance value to consider faces with near-zero area as degenerate.

    Returns:
        torch.Tensor: A boolean tensor indicating which faces are degenerate.
    """
    pos, face = data.pos, data.face

    # Get the vertex positions for each face
    v0 = pos[face[0]]
    v1 = pos[face[1]]
    v2 = pos[face[2]]

    # Compute the edge vectors for each face
    e1 = v1 - v0
    e2 = v2 - v0

    # Compute the cross-product to get the face normal and area
    normals = torch.cross(e1, e2, dim=1)
    areas = torch.norm(normals, p=2, dim=1)

    # A face is degenerate if its area is close to zero
    return areas < tol


class FaceDistance(nn.Module):
    """
    Compute edge distances from face (triangle) information using PyG utilities.
    """

    def __init__(self, loop=True):
        super(FaceDistance, self).__init__()
        from torch_geometric.transforms import FaceToEdge

        self.loop = loop
        self.transform = FaceToEdge(remove_faces=False)

    def forward(self, pos, faces, batch=None, entry_dir=None):
        """
        Extract undirected edges from faces.
        """
        # Ensure faces is [3, num_faces]
        if faces.shape[0] != 3:
            faces = faces.t()

        # Extract all edges from triangles
        res_data = self.transform(Data(pos=pos, face=faces, num_nodes=pos.size(0)))
        edge_index = res_data.edge_index

        # Make undirected and remove duplicates
        edge_index = to_undirected(edge_index, num_nodes=pos.size(0))

        # Filter edges if batch is provided
        ...
        # Debug: Check for problematic edges
        zero_edges = edge_weight < 1e-8
        # if zero_edges.any():
        #     print(f"[FaceDistance] Found {zero_edges.sum()} zero-length edges")
        #     print(f"[FaceDistance] Zero edge indices: {edge_index[:, zero_edges]}")
        #     print(
        #         f"[FaceDistance] Zero edge positions: {pos[edge_index[0, zero_edges]]}"
        #     )
        #     if entry_dir is not None:
        #         print(f"[FaceDistance] Zero edge entry directions: {entry_dir}")

        nan_edges = torch.isnan(edge_vec).any(dim=1)
        if nan_edges.any():
            print(f"[FaceDistance] Found {nan_edges.sum()} edges with NaN vectors")
            print(f"[FaceDistance] NaN edge indices: {edge_index[:, nan_edges]}")

        # Fix: Remove problematic edges
        valid_edges = ~(zero_edges | nan_edges)
        if not valid_edges.all():
            # print(f"[FaceDistance] Removing {valid_edges.sum()} problematic edges")
...
class GotenNetV2Wrapper(GotenNetV2):
    """
    The wrapper around GotenNet for processing atomistic data.
    """

    def __init__(self, *args, max_num_neighbors=1000, **kwargs):
        super(GotenNetV2Wrapper, self).__init__(*args, **kwargs)

        self.distance = FaceDistance(loop=False)
        self.reset_parameters()

    def forward(
        self, inputs: Mapping[str, Tensor], entry_dir=None
    ) -> Tuple[Tensor, Tensor]:
        """
        Compute atomic representations/embeddings.
        ...
        Returns:
            Tuple containing:
                ...
                - High-degree steerable features [num_nodes, (L_max ** 2) - 1, hidden_dims]
        """
        node_features, pos = inputs.z, inputs.pos
        batch = inputs.batch

        edge_index, edge_diff, edge_vec = self.distance(
            pos, inputs.faces, batch, entry_dir
        )
        return super().forward(node_features, edge_index, edge_diff, edge_vec)


class GotenNetV3Wrapper(GotenNetV3):
    """
    The wrapper around GotenNetV3 for processing atomistic data.
    """

    def __init__(self, *args, max_num_neighbors=1000, **kwargs):
        super(GotenNetV3Wrapper, self).__init__(*args, **kwargs)

        self.distance = FaceDistance(loop=True)
        self.reset_parameters()
    ...
    def forward(self, inputs: Mapping[str, Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        node_features, pos = inputs.z, inputs.pos
        batch = inputs.batch
        drug = inputs.drug if not self.use_concise else inputs.codes

        edge_index, edge_diff, edge_vec = self.distance(pos, inputs.faces, batch)
        return super().forward(
            node_features, edge_index, edge_diff, edge_vec, drug, batch
        )
