...
class NodeInitFeat(MessagePassing):
    """
    Node initialization variant that uses provided node features instead of an embedding.

    This layer mirrors `NodeInit` but replaces the neighbor embedding lookup with
    the given node feature tensor. It projects the concatenation of the initial
    node features and aggregated neighbor messages to the hidden representation.

    Args:
        hidden_channels (Union[int, List[int]]): Hidden dimensions for the projection MLP.
        num_rbf (int): Number of radial basis functions used for edge features.
        cutoff (float): Cutoff distance for interactions.
        activation (Callable, optional): Activation function. Defaults to F.silu.
        proj_ln (str, optional): Type of layer normalization for projection ('layer' or ''). Defaults to ''.
    ...
    def __init__(
        ...
        super(NodeInitFeat, self).__init__(aggr="add")
        if type(hidden_channels) == int:
            hidden_channels = [hidden_channels]

        last_channel = hidden_channels[-1]

        # Same radial processing as NodeInit
        self.W_ndp = MLP(
            [num_rbf] + [last_channel],
            activation=None,
            norm="",
        ...
        # Project concatenated [h, m_i] to hidden dims
        self.W_nrd_nru = MLP(
            [2 * last_channel] + hidden_channels,
            activation=activation,
            norm=proj_ln,
    ...
    def forward(self, h, edge_index, r0_ij, varphi_r0_ij):
        """
        Args:
            h (Tensor): Node feature tensor of shape [num_nodes, hidden_dim].
            edge_index (LongTensor): Graph connectivity [2, num_edges].
            r0_ij (Tensor): Edge distances [num_edges].
            varphi_r0_ij (Tensor): Radial basis expansion of distances [num_edges, num_rbf].
        Returns:
            Tensor: Updated node features [num_nodes, hidden_dim].
        """
        # remove self loops for the distance-dependent message
        mask = edge_index[0] != edge_index[1]
        if not mask.all():
            edge_index = edge_index[:, mask]
            r0_ij = r0_ij[mask]
