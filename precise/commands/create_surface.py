...
def _create_surface(
    ...
    Args:
        ...
        chain_id: Chain identifier(s) to process (comma-separated for multimer, None for auto-detect all chains)
    ...
    """
    from precise.preprocess.msms_pipeline import process_protein_auto, detect_chains

    if not pdb_path.exists():
        raise FileNotFoundError(f"PDB file not found: {pdb_path}")

    if chain_id is None:
        chain_ids = detect_chains(pdb_path)
    else:
...
def register_command(app: typer.Typer, console: Console):
    ...
    def create_surface(
        ...
        chain_id: Annotated[
            str,
            typer.Option(
                help="Chain identifier(s) to process (comma-separated for multimer, omit for auto-detect all)",
            ),
        ] = None,
        color: Annotated[
            bool,
        ...
        Examples:
            ...
            precise create-surface --pdb-path protein.pdb --output-path surface.ply

            # Specify single chain
            precise create-surface --pdb-path protein.pdb --chain-id A --output-path surface.ply

            # Specify multiple chains for multimer
            precise create-surface --pdb-path protein.pdb --chain-id "A,B" --output-path surface.ply
        """
        ...
        console.print(f"  Chain ID(s): {chain_id if chain_id else 'All'}")
        if output_path:
