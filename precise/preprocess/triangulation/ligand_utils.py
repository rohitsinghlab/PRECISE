...
def calculate_mol2(pdb_input, mol2_outfile):
    """
    Convert PDB format to MOL2 format for a ligand.
    
    Parameters:
    -----------
    pdb_input : str
        Either a PDB filename (path to file) or a PDB format string
    mol2_outfile : str
    ...
    try:
        # First try as a file
        if obConversion.ReadFile(ob_mol, pdb_input):
            # Successfully read as file
            pass
        else:
            # Try as string
            if not obConversion.ReadString(ob_mol, pdb_input):
                print(f"Error: Could not read PDB input")
                return False
    except:
        # If ReadFile fails, try as string
        if not obConversion.ReadString(ob_mol, pdb_input):
            print(f"Error: Could not read PDB input")
            return False
    
    # Write to mol2 file
    if not obConversion.WriteFile(ob_mol, mol2_outfile):
        print(f"Error: Could not write MOL2 file to {mol2_outfile}")
        return False
    
    # Remove amide bond type (not supported by PDB2PQR)
    amide_to_single_bond(mol2_outfile)
