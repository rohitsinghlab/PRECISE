...
from precise.preprocess.default_config.global_vars import (
    apbs_bin,
    pdb2pqr_bin,
    multivalue_bin,
)
...
def computeAPBS(vertices, pdb_file, tmp_file_base, mol2_file=None):
    ...
    if mol2_file is not None and metal_info is None:
        ...
        print(
            f"Detected metal ion: {metal_info['res_name']}. Running PDB2PQR without --ligand."
        )
    ...
    if metal_info is not None:
        try:
            ...
            if last_atom_id == 0:
                logger.warning(
                    "Could not find last atom ID in PQR file. Defaulting to 0."
                )
