...
def fix_protein_mesh(
    ...
    logger.info(
        f"Target: {target_vertices} vertices (range: {min_vertices}-{max_vertices})"
    )
    ...
    if current_vertices > max_vertices:
        ...
        for step_target in reduction_steps:
            ...
            try:
                mesh = mesh.simplify_quadric_decimation(
                    ...
                    else 8,  # 1-10, higher = more aggressive
                )
            except:
        ...
        logger.info(
            f"Subdividing {subdivisions_needed} times to increase vertex count..."
        )
        ...
        if len(mesh.vertices) > max_vertices:
            target_faces = target_vertices * 2
            mesh = mesh.simplify_quadric_decimation(
                face_count=target_faces, aggression=7 if preserve_features else 8
            )
    ...
    bad_faces_mask = np.any(face_angles > max_angle_threshold, axis=1) | np.any(
        face_angles < min_angle_threshold, axis=1
    )

    if (
        np.any(bad_faces_mask) and np.sum(~bad_faces_mask) > 100
    ):  # Keep at least 100 faces
    ...
    if (
        len(mesh.vertices) < min_vertices * 0.8
        or len(mesh.vertices) > max_vertices * 1.2
    ):
...
def fix_mesh_by_edge_length(mesh, target_edge_length):
    ...
    ms.meshing_remove_connected_component_by_diameter(
        mincomponentdiag=pymeshlab.PureValue(1.0)
    )

    ms.meshing_isotropic_explicit_remeshing(
        targetlen=pymeshlab.PureValue(target_edge_length), iterations=10
    )

    ms.apply_filter(
        "apply_coord_laplacian_smoothing", stepsmoothnum=2, cotangentweight=True
    )
